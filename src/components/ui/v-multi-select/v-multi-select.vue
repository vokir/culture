<template>
  ...
  <div class="select__container" ref="select">
    ...
    <v-dropdown class="select__filter" 
    :delay="{show: 0,hide: 0}" 
    :triggers="[]" 
    :popperTriggers="[]" 
    :shown="isOpened"  
  ...
  setup(props,context){
    ...
    const optionsFiltered = computed(() => {
      return props.options.filter(option => {
        return (selectedList.value.every(item => option.ID !== item.ID) && option.FULL_NAME.includes(searchStr.value))
      })
    })

    const filterTable = () => {
      isOpened.value = false
      context.emit('filterTable', JSON.parse(JSON.stringify(options.value)), search.value)
    }

    const clearFilter = () =>{
      if(search.value || selectedList.value){
        search.value = ''
        selectedList.value = []
        filterTable()
      }
