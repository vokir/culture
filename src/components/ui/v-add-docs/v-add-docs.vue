<template>
  <v-tags-list :label="label" :tags="modelValue" @open-modal="openModal" @remove-tag="removeDocument">
    <v-modal
      v-if="isOpen"
      ref="modalRef"
    ...
    >
      <div class="docs-wrapper">
        ...
        <v-table v-else :rows="docs" @row-click="setSelectedDocs">
          <v-table-column id="UF_TITLE" title="Название" width="190" class="column-name">
        ...
        <div class="docs-wrapper__actions">
          <v-button variant="gray" @click="closeModalRef">Выбрать документ</v-button>
  ...
  setup({ modelValue }, { emit }) {
    ...
    const docs = computed(() => {
      return store.documents.map((doc) => ({
        ...doc,
        selected: !!selectedDocs.value.find((sDoc) => doc.ID === sDoc.ID)
      }));
    });
    ...
    documentFieldsPromise.then((schemaFields) => {
      ...
      schemaFields.map((field) => {
        if (filtersName.includes(field.name)) {
          ...
          switch (field.name) {
            case 'type':
              (newField.type = 'select'), (newField.load = store.loadDocumentTypes);
