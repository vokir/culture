<template lang="">
  <div class="news__for">
    ...
    <div class="for__row" v-for="row in rows" ref="rowsDiv">
      <div v-for="cell in row" class="for__cell" :data-id="cell.ID">{{ cell.name }}</div>
      <div  v-for="premise in cell" class="for__cell" :data-id="cell.ID">{{ cell.name }}</div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  setup(props) {
    ...
    if (props.newsInfo.premises.length > 0) {
      ...
      premises.forEach((el) => {
        ...
        const rowsPush = (_) => {
          ...
          rows.push([
            ...
            {
              name: getApproacheNumber(el.floor.approache.UF_NAME),
      ...
      approaches.forEach((el) => {
        const rowsPush = (_) => {
          ...
          rows.push([el.house.UF_NAME, getApproacheNumber(el.UF_NAME)]);
        };
    ...
    onMounted(() => {
      // console.log(setHouses);
      // console.log(setApproaches);
      // console.log(setFloors);

      rowsDiv.value.forEach((row) => {
        let cells = row.querySelectorAll(".for__cell");
        cells.forEach((cell) => {
          // console.log(props.newsInfo.houses);
