...
const useFilter = (entity = '', props, emit) => {
  ...
  const computedFields = computed(() => {
    return props.fields.map((field) => {
      if (field.checked && field.load) {
        field.load();
        field.computedResult = field.result.map((obj) => {
          return { ...obj, name: obj.UF_NAME || obj.UF_TITLE };
        });
        if (field.type === 'select') {
          if (field.computedResult.length) {
            field.value = field.value
              ? field.computedResult.filter((row) => row.ID === field.value?.ID)[0]
              : null;
        ...
        if (field.options.length && field.value) {
          ...
          field.selectedOption = field.options.find(
            (option) => option.name === tempValue.value.name
          );
        }
      }
      return field;
  ...
  const checkedFields = computed(() => {
    return props.fields.filter((field) => field.checked).sort((a, b) => a.order - b.order);
  });
  ...
  const updateFieldsWithValue = (fieldsArg) => {
    ...
    if (selectedFilter.value.fields) {
      ...
      if (
        selectedFilter.value.fields &&
        JSON.stringify(selectedFilterValues) === JSON.stringify(fieldsValue)
      ) {
        fieldsWithValue.value[0].filter = true;
      } else {
        curFields.map((propField) => {
          const filterField = selectedFilter.value.fields.find(
            (field) => field.name === propField.name
          );
    ...
    fieldsWithValue.value = fieldsWithValue.value.filter((field) =>
      !field.filter ? fieldHasValue(field) : true
    );
    fieldsWithValue.value
  ...
  const addFilter = (name) => {
    ...
    newFilter.value = {
      ...
      order: filtersTemp.value.length,
      newName: name
    };
  ...
  const saveFilters = () => {
    ...
    filtersTemp.value.map((filter) => {
      ...
      updateFilter({
        id: filter.ID,
        content: JSON.stringify({ ...filter, selected: false }),
        entity: entity
      });
    });
    store.refetchFiltersNews();
    selectedFilter.value.filter
      ? (defaultFilterFields.value = JSON.parse(JSON.stringify(selectedFilter.value.fields)))
      : null;
  };
  ...
  const setPin = (filter) => {
    if (filter.pinned) {
      ...
      filtersTemp.value.map((row) => {
        ...
        updateFilter({
          id: row.ID,
          content: JSON.stringify({ ...row, selected: false }),
          entity: entity
        });
      });
    }
  ...
  const removeFilter = (id) => {
    ...
    filtersBeforeChanges.value.splice(
      filtersBeforeChanges.value.indexOf(exactFilterBeforeChanges),
      1
    );
  ...
  const toggleChangingSettings = (bool) => {
    !changingSettings.value
      ? (filtersBeforeChanges.value = JSON.parse(JSON.stringify(filtersTemp.value)))
      : null;
    changingSettings.value = bool !== undefined ? bool : !changingSettings.value;
  };

  watch(
    [selectedFilter, changingSettings],
    ([newSelectedFilter, newChangingSettings], [oldSelectedFilter, oldChangingSettings]) => {
      ...
      if (
        newSelectedFilter !== oldSelectedFilter &&
        oldSelectedFilter &&
        changingSettings.value &&
        filterToUnlink
      ) {
    ...
    if (!selectedFilter.value.pinned && pinnedFilter) {
      if (fieldsWithValue.value.length > 1 && Object.keys(selectedFilter.value).length) {
        ...
        const fieldsWithoutFilter = props.fields.map((field) => {
          const curField = JSON.parse(
            JSON.stringify(
              selectedFilter.value.fields.find((filterField) => field.name === filterField.name)
            )
          );
